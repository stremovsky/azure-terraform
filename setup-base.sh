...
WORKLOAD_CLIENT_ID=$(echo "$TERRAFORM_OUTPUT" | jq -r '.workload_identity_client_id.value')
...
if helm status base --namespace default &> /dev/null; then
    echo "Helm release 'base' is already installed."
    helm upgrade base ./base-chart \
